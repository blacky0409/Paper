...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.95\columnwidth]{graphs/aging-duration}
...
Figure~\ref{fig:aging-duration} represents the number of rounds it takes for the storage to age.
Each round takes 10 minutes.
We measured the time taken when the storage sizes are 16G, 32G, and 128G, and observed that as the size increases, the time required for aging also increases.
In our experiment, we conducted tests in a small storage scenario, yet it still required a significant amount of time
