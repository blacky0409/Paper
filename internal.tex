...
To reflect the inherent parallelism of modern SSDs, NVMeVirt is comprised of multiple FTL instances and each instance emulates the full hierarchy of core storage hardware components such as PCIe links, channels, dies, blocks, and pages.
NVMeVirt enables realistic data processing by coordinating the channels and dies.
Thus, although data transmission is serialized over the PCIe link, the FTL handles data I/O requests in parallel across multiple NAND channels and dies.
...
Figure~\ref{fig:making} illustrates the process for instantizing a virtual NVMe device.
During system initialization, a portion of the physical memory range is reserved and used to store both user data and management metadata.
Users can specify the type of device to emulate along with the memory range (start and end addresses) via a configuration file.
Internally, the dispatcher and I/O worker interact to process data like a native NVMe device.
This infrastructure allows researchers to instantiate multiple device instances, facilitating experimentation with analysis of diverse SSD architectures.

% Space
Most of the reserved memory is used for storing user data.
The physical memory range corresponds to the logical block address (LBA) space.
On top of the LBA address space, each storage type FTL emulates the location of LBA in physical address space.
While locating the position, FTL computes the time to complete the request.
...
In NVMeVirt, there are four main types of data that need to be retrieved to make a snapshot from an aged SSD.
% 1. FTL
