...
As discussed in the previous section, fragmentation continues to pose challenges even in modern storage systems.
In the past, its implications were relatively straightforward in the context of HDDs.
However, with the increasing complexity of host-side I/O stack and device architecture internals, the effects of fragmentation have become more nuanced.
In particular, logical and physical fragmentation can now occur independently, and each type requires different strategies for mitigation.

% Logical fragmentation: faster storage performance
Moern operating systems feature highly optimized I/O stacks designed to fully utilize high-performance storage devices.
Fragmentation disrupts data contiguity, causing a single logical I/O request to be split into multiple smaller I/O requests~\cite{IO}.
This increases the total number of I/O requests.
All sub-requests generated for one I/O request must complete before the request is considered finished.
Therefore, this division of I/O request introduces latency and synchronization bottlenecks.
Moreover, for each fragmented I/O, the operating system must allocate and manage multiple data structures, such as bio structures, request queues, and I/O commands.
These additional operations incur overhead, undermining the efficiency of the I/O stack and degrading overall performance.


% Physical fragmentation: complex internal for parallelism
Secondly, SSDs enhance read performance through prefetching, which loads subsequent data in advance based on LBA.
Fragmentation can cause this prefetching to load unnecessary data.\cite{defrag-lfs:apsys16} For these reasons, filesystem fragmentation is also a concern in SSDs.
and the internal architectures of modern storage devices getting sophisticated.
